...
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_boost_var_00');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_boost_var_00');
            }
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_boost_var_00');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_boost_var_00');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_boost_var_00',
